...
<div id="actionToast" data-current-user-id={userId ?? ""} style={{top: isActionToastVisible ? "0" : "-150px", whiteSpace: "pre-line"}}>{actionToastText}</div>
  ...
  <StyleOnboardingModal
    isOpen={isOnboardingStyleModalOpen}
    lineText={onboardingLineText}
    onLineTextChange={setOnboardingLineText}
    onBackdropClose={() => setIsOnboardingStyleModalOpen(false)}
    onSubmit={() => executeStyleAnalysis(onboardingLineText)}
  />
  ...
  <CustomerEditModal
    ...
    isEditingHiddenCustomer={isEditingHiddenCustomer}
    onOpenHiddenDeleteConfirm={openHiddenDeleteConfirm}
    isCreateCustomerMode={isCreateCustomerMode}
    isEditingDummyCustomer={isEditingDummyCustomer}
    editCustomerName={editCustomerName}
    onEditCustomerNameChange={setEditCustomerName}
    isTagAccordionOpen={isTagAccordionOpen}
    onTagAccordionToggle={() => setIsTagAccordionOpen((isOpen) => !isOpen)}
    editAttributeOptions={editAttributeOptions}
    editAttributeTags={editAttributeTags}
    onToggleAttributeTag={(tag) => toggleStringValue(tag, setEditAttributeTags)}
    customAttrInput={customAttrInput}
    onCustomAttrInputChange={setCustomAttrInput}
    ...
    addNewMemoBlock={addNewMemoBlock}
