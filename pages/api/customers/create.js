...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data: created, error } = await getSupabase().from('customers').insert({ user_id: userId, name: newName, tags: tagsArray }).select('id, name, tags').single();
    if (error) throw new Error('Supabase保存エラー: ' + error.message);
    return sendJson(res, 200, { success: true, customer: { id: created.id, name: created.name, tags: normalizeTags(created.tags).join(', ') } });
