...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data: existingFavorite, error: existingFavoriteError } = await supabase.from('favorite_writing_samples').select('id').eq('user_id', userId).eq('source_entry_id', String(entryId)).maybeSingle();
    if (existingFavoriteError) throw new Error('お気に入り確認に失敗しました');
    if (existingFavorite?.id) { const { error } = await supabase.from('favorite_writing_samples').delete().eq('id', existingFavorite.id).eq('user_id', userId); if (error) throw new Error('お気に入り解除に失敗しました'); return sendJson(res, 200, { success: true, isFavorited: false }); }
    const { count, error: countError } = await supabase.from('favorite_writing_samples').select('id', { count: 'exact', head: true }).eq('user_id', userId);
    if (countError) throw new Error('お気に入り件数の確認に失敗しました');
    if ((count || 0) >= 5) return sendJson(res, 200, { success: true, isFavorited: false, limitReached: true });
    ...
    const { error } = await supabase.from('favorite_writing_samples').insert({ user_id: userId, source_entry_id: String(entry.id), source_customer_id: sanitizeId(entry.customer_id) || customerId, source_customer_name: customerName, sample_text: sampleText, updated_at: new Date().toISOString() });
    if (error) throw new Error('お気に入り登録に失敗しました');
    return sendJson(res, 200, { success: true, isFavorited: true });
  } catch (err) { console.error('favorites/toggle error:', err); return sendJson(res, 500, safeApiError()); }
}
